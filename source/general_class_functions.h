...
std::deque <node*> get_parent_pool(string sex,int mat_age_m,int mat_age_f,int gest_length, std::deque <node>*all_nodes_ptr, node* indiv, string nonparents = "");
string all_nodes_to_population_file(std::deque <node> *all_nodes_ptr,string filename);
std::deque<double> get_all_gaps(std::deque<gap>*gaps_ptr,std::deque<node>*all_nodes_ptr,int default_year);
void get_start_solution_all_gaps(double temperature,std::deque<gap>*gaps_ptr,std::deque<node>*all_nodes_ptr,std::deque<node>*current_nodes_ptr,int maturation_age_m,int maturation_age_f,int gestation_length,int default_year);
