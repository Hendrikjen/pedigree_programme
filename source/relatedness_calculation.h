...
double calculate_f_xy(node * indiv_1,node * indiv_2,std::deque<std::deque<double>> *f_matrix,int ncol,int nrow,std::deque <node> * all_nodes_ptr,std::deque <dyad> * all_dyads_ptr,int dyad_index,std::deque <int> * path_indices_ptr,int path_index,map <string,int> * dyad_dict_ptr, std::set<string>* node_space_ptr);
...
void pip_forward(string file,string output_file,string input_dyadlist,int maturation_age_f,int maturation_age_m,int gestation_length,string write_dyadlist,int generation_limit = -1,int n_cores = 1,bool reduce_node_space_tf = false);
