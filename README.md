...
This is highly relevant, as illunstrated in teh following example. Assume a DNA sample of a male is missing, but he sired two offspring. Hence these two infants related over this maler could not have been identified in the pedigree. In this example, the simulated annealing approach would perceive the discrepancy of the pedigree-derived relatedness coefficient of the paternal halfsiblings, that is 0 since the father is unknown, and the provided realised relatedness value of something around 0.25. Optimally, that would result in identifying one individual as the father of both infants (e.g. labled as 'male x')  while validating the missing father by realised relatedness values of further individuals related or unrelated to either the potential father or the half-siblings. Hence, the realised relatedness values are passed to the algorithm, the higher the probability of getting a correctly reconstructed pedigree BEST FILLED WITH LOWEST DISCREPANCY? WE DO KNOW WHAT IS THE CORRECT PEDIGREE. Therefore, even if the ID of the father is unknown due to missing DNA, he can be identified based on whole genome sequencing data to his descendants (although his ID is still unknown, requiring to lable him 'male x'). COULD YOU WRITE THIS CLEARER? THIS IS HOW I UNDERSTOOD IT: BASED ON WGS WE KNOW THEY HAVE THE SAME FATHER; BUT WE DO KNOW THE ID OF THE FATHER: HOWEVER, BASED ON THE WGS INFO THE PEDIGREE CAN BE FILLED AND OTHER KIN/NONKIN towards the TWO INFANTS CAN BE ESTABLISHED) 
...
- Create a start solution by randomly assigning parents from a pool of suitable candidates for each gap. Suitable candidates are parents who were alive and mature at the time of conception (sire) or birth (mother) and were not excluded as potential parents priorly due to genetic analysis DEFINE NONSIRES/NONDAMS HERE? or because a female may already an offspring in the respective cohort (quite unlikely as twins are really rare) 
...
- columns (order and format are mandatory): ID, sex, birth season/year, mom_ID, sire_ID, day of birth (DOB), day of death (DOD), nonsire, nondam
...
We want to thank and express our deep gratitude to all the people who supported and contributed to this project AW: LIST THEM HERE, with special thanks to R. McElreath and P. Fröhlich for the provided server space and comprehensive IT support.
